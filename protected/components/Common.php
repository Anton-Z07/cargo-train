...
class Common
{
	...
	{
		return substr(sha1(microtime(1) . rand() . 'jio2378bnjk'), 3, 8);
	}
	...
	public static function getUser()
	{
		...
		if (Yii::app()->user->type == 'user')
			self::$user = User::model()->findByPk(Yii::app()->user->innerId);
		elseif (Yii::app()->user->type == 'partner')
			self::$user = Partner::model()->findByPk(Yii::app()->user->innerId);
		return self::$user;
	}
	...
	{
		return substr($city_id, 0, 2) . '00000000000';
	}

	public static function minutesToTime($m)
	{
		$t = '';
		if ($m > 60) {
	        $h = floor($m / 60);
	        $t =  $h . 'ч '; //Common::getWordForm($h, ' час ', ' часа ', ' часов ');
	        $m %= 60;
	    }
	    if ($t) $t .= $m . 'м'; //Common::getWordForm($m, ' минута', ' минуты', ' минут');
	    else $t = $m . ' мин';
	    return $t;	
	}

	public static function getYearIsDiff($date)
	{
	...
	{
		Yii::app()->session['redirect_url'] = $url;
	}
